        ...
        void FixedUpdate()
        {
            if (Controller != null && !start)
            {
                ...
                Direction = Quaternion.AngleAxis(
                                Controller.AngleX + a + (Controller.PositionType == PositionType.Relative
                                    ? Controller.transform.rotation.eulerAngles.y
                                    : 0), Vector3.up) *
                            Quaternion.AngleAxis(
                                Controller.AngleY + b + (Controller.PositionType == PositionType.Relative
                                    ? -Controller.transform.rotation.eulerAngles.x
                                    : 0), Vector3.left);
            ...
            if (start)
            {
                ...
                time++;
                if (Controller.DestroyType == DestroyType.Distance &&
                    Vector3.Distance(Controller.transform.position, transform.position) > Controller.Distance ||
