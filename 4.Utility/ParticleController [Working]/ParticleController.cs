...
namespace MyLibrary.Utility
{
	...
	public class ParticleController : MonoBehaviour
	{
		...
		[Range(0, 360)] public float AngleX = 0f;
		[Range(0, 360)] public float AngleY = 0f;
		[Range(1, 40)] public float Radius = 10f;
		[Range(0, 10)] public float Distance = 1f;
		public VolumeParticle ParticleMesh;

		[Range(0, 1)] public float PositionOffset = 0;

		[Min(0)] public float Speed = 50f;
		[Min(0)] public float SpeedOffset = 0;

		[Min(0)] public float Count = 30;
		[Min(0)] public float CountOffset = 0;
		internal float AdditionalAngle = 0;

		[Min(0)] public float SizeOffset = 0;
		...
		void OnDrawGizmosSelected()
		{
			Quaternion a =
				Quaternion.AngleAxis(
					AngleX + (PositionType == PositionType.Relative ? transform.rotation.eulerAngles.y : 0),
					Vector3.up);
			Quaternion b =
				Quaternion.AngleAxis(
					AngleY + (PositionType == PositionType.Relative ? -transform.rotation.eulerAngles.x : 0),
					Vector3.left);
